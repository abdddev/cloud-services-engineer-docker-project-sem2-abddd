...
Trivy проверяет образы на известные уязвимости в системных пакетах и зависимостях.

Рекомендуемые уровни для CI:

```text
CRITICAL,HIGH
```

Пример назначения job:

```text
trivy-scan
```

Если Trivy находит уязвимости уровня `HIGH` или `CRITICAL`, pipeline должен завершаться с ошибкой. Это помогает не публиковать заведомо уязвимые образы.
